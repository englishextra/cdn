			...
			(function (global, factory) {
				...
				var utils = {
					...
					setVendor: function setVendor(el, property, value) {
						...
						el.style[property.charAt(0)
							.toLowerCase() + property.slice(1)] = value;
									...
									if (_event.substring(0, 1) === '.') {
										for (var key in utils.Listener) {
											if (utils.Listener.hasOwnProperty(key)) {
												if (key.split('.')
													.indexOf(_event.split('.')[1]) > -1) {
													el.removeEventListener(key.split('.')[0], utils.Listener[key]);
													el.setAttribute('lg-event-uid', el.getAttribute('lg-event-uid')
														.replace('&' + _id[i], ''));
		...
		2: [function (require, module, exports) {
			(function (global, factory) {
					...
					if (this.s.dynamic) {
						...
						if (this.s.selector === 'this') {
							...
							if (this.s.selectWithin) {
								this.items = document.querySelector(this.s.selectWithin)
				...
				Plugin.prototype.structure = function () {
					...
					if (this.s.download) {
						this.outer.querySelector('.lg-toolbar')
							.insertAdjacentHTML('beforeend', '<a id="lg-download" target="_blank" download class="lg-download lg-icon"></a>');
				...
				Plugin.prototype.counter = function () {
					if (this.s.counter) {
						this.outer.querySelector(this.s.appendCounterTo)
							.insertAdjacentHTML('beforeend', '<div id="lg-counter"><span id="lg-counter-current">' + (parseInt(this.index, 10) + 1) + '</span> / <span id="lg-counter-all">' + this.items.length + '</span></div>');
					}
				};
				/**
					...
					if (this.s.dynamic) {
						...
						if (this.s.getCaptionFromTitleOrAlt && !subHtml) {
							...
							if (subHtml && currentEle.querySelector('img')) {
								subHtml = currentEle.querySelector('img')
									.getAttribute('alt');
							}
						}
					}
					...
					if (typeof subHtml !== 'undefined' && subHtml !== null) {
						...
						if (fL === '.' || fL === '#') {
							if (this.s.subHtmlSelectorRelative && !this.s.dynamic) {
								subHtml = currentEle.querySelector(subHtml)
									.innerHTML;
							} else {
								subHtml = document.querySelector(subHtml)
									.innerHTML;
							}
						}
					} else {
				...
				Plugin.prototype.loadContent = function (index, rec, delay) {
					...
					if (_this.s.dynamic) {
						...
						if (_this.items[index].getAttribute('data-responsive')) {
							var srcItms = _this.items[index].getAttribute('data-responsive')
								.split(',');
				...
				Plugin.prototype.slide = function (index, fromTouch, fromThumb) {
					...
					if (!_this.lgBusy) {
						if (this.s.download) {
							...
							if (_src) {
								document.getElementById('lg-download')
						...
						try {
							...
							if (document.querySelector('.lg-backdrop')) {
								document.querySelector('.lg-backdrop')
									.parentNode.removeChild(document.querySelector('.lg-backdrop'));
							}
