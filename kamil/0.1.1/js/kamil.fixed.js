...
;(function (window, document) {
	...
	var $ = {
		...
		getItemValue: function (kamil, listItem) {
			var item = kamil._data[parseInt(listItem.getAttribute('data-position'), 10)],
		...
		setActive: function (o) {
			...
			$.trigger(this._menu, 'kamilfocus', {
				item: this._data[parseInt(o.item.getAttribute('data-position'), 10)],
		...
		move: function (direction) {
			...
			if (direction === 'previous' && $.isActive(this._menu, 'first') || direction === 'next' && $.isActive(this._menu, 'last') || direction === 'previous' && this._activeIndex === 1 && items[0].classList.contains(this._opts.exclude)) {
			...
			if (items[this._activeIndex] && items[this._activeIndex].classList.contains(this._opts.exclude)) {
				this._activeIndex = this._activeIndex + (direction === 'previous' ? -1 : 1);
			}
			$.setActive.call(this, {
	...
	var handlers = {
			...
			return function (e) {
				...
				case 13:
					...
					$.trigger(k._menu, 'kamilselect', {
						item: k._data[parseInt(activeItem.getAttribute('data-position'), 10)],
		...
		'mouseup': function (k) {
			return function () {
				...
				$.trigger(k._menu, 'kamilselect', {
					item: k._data[parseInt(this.getAttribute('data-position'), 10)],
					inputElement: k._srcElement
				});
				k._srcElement.focus();
				k.close();
