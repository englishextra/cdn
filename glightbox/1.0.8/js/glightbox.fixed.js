...
(function(global, factory) {
...
})("undefined" !== typeof window ? window : void 0, function(module) {
	"use strict";
	...
	function addEvent(eventName) {
		var _ref =
			...
			avoidDuplicate =
				_ref$avoidDuplicate === undefined ? true : _ref$avoidDuplicate,
			_ref$once = _ref.once,
			once = _ref$once === undefined ? false : _ref$once,
			_ref$useCapture = _ref.useCapture,
			useCapture =
				_ref$useCapture === undefined ? false : _ref$useCapture;
	...
	function hasClass(node, name) {
		return node.classList
			...
			: new RegExp("(^| )" + name + "( |$)", "gi").test(node.className);
	}
	/**
	...
	var getSlideData = function getSlideData() {
		...
		data.sourcetype = data.hasOwnProperty("type")
			? data.type
			: data.sourcetype;
		data.type = data.sourcetype;
		var defaultWith =
			data.sourcetype == "video" ? settings.videosWidth : settings.width;
		var defaultHeight =
			data.sourcetype == "video"
				? settings.videosHeight
				: settings.height;
	...
	function setSlideVideo(slide, data, callback) {
		...
		if (source == "local") {
			...
			if (
				this.settings.jwplayer !== null &&
				this.settings.jwplayer.api !== null
			) {
	...
	function touchNavigation() {
		...
		this.events["touchStart"] = addEvent("touchstart", {
			...
			withCallback: function withCallback(e, target) {
				...
				activeSlideDesc = activeSlide.querySelector(
					".gslide-description"
				);
				index = _this4.index;
		...
		this.events["touchMove"] = addEvent("touchmove", {
			...
			withCallback: function withCallback(e, target) {
				...
				e.preventDefault();
				endCoords = e.targetTouches[0];
				var slideHeight = activeSlide.querySelector(
					".gslide-inner-content"
				).offsetHeight;
				var slideWidth = activeSlide.querySelector(
					".gslide-inner-content"
				).offsetWidth;
				...
				if (hSwipe) {
					if (
						_this4.index + 1 == _this4.elements.length &&
						hDistance < -60
					) {
		...
		this.events["touchEnd"] = addEvent("touchend", {
			...
			withCallback: function withCallback(e, target) {
				...
				var slideWidth = activeSlide.querySelector(
					".gslide-inner-content"
				).offsetWidth; // Swipe to top/bottom to close
		...
		_createClass(GlightboxInit, [
			...
			},
			{
				...
				value: function showSlide() {
					...
					show(this.loader);
					this.index = index;
					var current = this.slidesContainer.querySelector(
						".current"
					);
					...
					var slide = this.slidesContainer.querySelectorAll(
						".gslide"
					)[index];
			...
			},
			{
				...
				value: function preloadSlide(index) {
					var _this7 = this; // Verify slide index, it can not be lower than 0
			...
			},
			{
				...
				value: function slideAnimateIn(slide, first) {
					...
					if (slideMedia.offsetWidth > 0 && slideDesc) {
						...
						slide.querySelector(
							".ginner-container"
						).style.maxWidth = slideMedia.offsetWidth + "px";
					...
					if (first) {
						animateElement(
							...
							function() {
								if (
									!isMobile &&
									_this8.settings.autoplayVideos
								) {
								...
								if (
									utils.isFunction(
										_this8.settings.afterSlideChange
									)
								) {
									_this8.settings.afterSlideChange.apply(
										_this8,
										[prevData, nextData]
									);
								}
						...
						animateElement(slide, animIn, function() {
							...
							if (
								...
								_this8.settings.afterSlideChange.apply(_this8, [
									prevData,
									nextData
								]);
							}
			...
			},
			{
				...
				value: function stopSlideVideo(slide) {
					if (utils.isNumber(slide)) {
						slide = this.slidesContainer.querySelectorAll(
							".gslide"
						)[slide];
					}

					var slideVideo = slide
						? slide.querySelector(".gvideo")
						: null;
