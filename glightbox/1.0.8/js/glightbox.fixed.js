...
(function (global, factory) {
	...
	function hasClass(node, name) {
		return node.classList ? node.classList.contains(name) : new RegExp("(^| )" + name + "( |$)", "gi")
			.test(node.className);
	}
	/**
	...
	function touchNavigation() {
		...
		this.events['touchMove'] = addEvent('touchmove', {
			...
			withCallback: function withCallback(e, target) {
				...
				e.preventDefault();
				endCoords = e.targetTouches[0];
				var slideHeight = activeSlide.querySelector('.gslide-inner-content')
					.offsetHeight;
				var slideWidth = activeSlide.querySelector('.gslide-inner-content')
					.offsetWidth;
		...
		_createClass(GlightboxInit, [{
			...
			value: function slideAnimateIn(slide, first) {
				...
				if (slideMedia.offsetWidth > 0 && slideDesc) {
					hide(slideDesc);
					slide.querySelector('.ginner-container')
