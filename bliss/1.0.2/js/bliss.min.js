/*!
 * Blissful JavaScript http://blissfuljs.com
 * github.com/leaverou/bliss/
 * blissfuljs.com/docs.html#vanilla
 * Adds $() and $$() global methods and a _ property on elements
 * and arrays, for convenience.
 * Also wraps addEventListener and removeEventListener
 * to track events. Perfect for when you control the environment.
 * Note: The $ is just an alias to Bliss.
 * If another library that uses $ is included before Bliss,
 * then Bliss will not use $.
 * You can always alias $ to Bliss manually,
 * by putting this before any of your code (but after including Bliss):
 * self.$ = Bliss;
 * Similarly, $$() is an alias to Bliss’ $.$() or Bliss.$() function.
 * If it’s already defined by the time Bliss is called,
 * Bliss will not use it.
 * You can do so manually with this line:
 * self.$$ = Bliss.$;
 * When writing Bliss plugins, if you want to use $() and $$()
 * it’s good practice to make local variables with them,
 * in case they’re not available:
