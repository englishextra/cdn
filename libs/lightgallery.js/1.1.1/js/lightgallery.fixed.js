	...
	return (function() {
						...
						var utils = {
							...
							on: function on(el, events, fn) {
								...
								events.split(" ").forEach(function(event) {
									var _id =
										el.getAttribute("lg-event-uid") || "";

									utils.Listener.uid++;
									_id += "&" + utils.Listener.uid;
									el.setAttribute("lg-event-uid", _id);
									utils.Listener[
										event + utils.Listener.uid
									] = fn;
									el.addEventListener(
										event.split(".")[0],
										fn,
								...
								if (_id) {
									...
									for (var i = 0; i < _id.length; i++) {
										if (_id[i]) {
											...
											if (
												_event.substring(0, 1) === "."
											) {
												for (var key in utils.Listener) {
													if (
														utils.Listener.hasOwnProperty(
															key
														)
													) {
														if (
				...
				function(require, module, exports) {
					(function(global, factory) {
						...
						function Plugin(element, options) {
							...
							this.isTouch =
								"ontouchstart" in document.documentElement; // Disable hideControlOnEnd if sildeEndAnimation is true
						...
						Plugin.prototype.build = function(index) {
							...
							for (var key in window.lgModules) {
								if (window.lgModules.hasOwnProperty(key)) {
									_this.modules[key] = new window.lgModules[
										key
									](_this.el);
								}
						...
						Plugin.prototype.structure = function() {
							...
							this.___slide = this.outer.querySelectorAll(
								".lg-item"
							);
							...
							if (this.doCss()) {
								var inner = this.outer.querySelector(
									".lg-inner"
								);
						...
						Plugin.prototype.setTop = function() {
							if (this.s.height !== "100%") {
								...
								var top =
									(wH - parseInt(this.s.height, 10)) / 2;
						...
						Plugin.prototype.isVideo = function(src, index) {
							...
							if (this.s.dynamic) {
								...
								html = this.items[index].getAttribute(
									"data-html"
								);
							}
						...
						Plugin.prototype.addHtml = function(index) {
							...
							if (this.s.dynamic) {
								...
								subHtml = currentEle.getAttribute(
									"data-sub-html"
								);
							...
							if (
								...
								if (fL === "." || fL === "#") {
									if (
										...
										subHtml = currentEle.querySelector(
											subHtml
										).innerHTML;
									} else {
										subHtml = document.querySelector(
											subHtml
										).innerHTML;
									}
								}
							...
							if (this.s.appendSubHtmlTo === ".lg-sub-html") {
								...
								this.___slide[index].insertAdjacentHTML(
									"beforeend",
									subHtml
								);
						...
						Plugin.prototype.loadContent = function(
							...
							var getResponsiveSrc = function getResponsiveSrc(
								srcItms
							) {
								var rsWidth = [];
								var rsSrc = [];
							...
							if (_this.s.dynamic) {
								...
								if (_this.s.dynamicEl[index].responsive) {
									var srcDyItms = _this.s.dynamicEl[
										index
									].responsive.split(",");
								...
								if (
									_this.items[index].getAttribute(
										"data-poster"
									)
								) {
									_hasPoster = true;
									_poster = _this.items[index].getAttribute(
										"data-poster"
									);
								}

								_html = _this.items[index].getAttribute(
									"data-html"
								);
								_src =
								...
								_srcset = _this.items[index].getAttribute(
									"data-srcset"
								);
								_sizes = _this.items[index].getAttribute(
									"data-sizes"
								);
							...
							if (
								...
								_img = _this.___slide[index].querySelector(
									".lg-object"
								);
							...
							_lgUtils2.default.on(
								_this.___slide[index].querySelector(
									".lg-object"
								),
								"load.lg error.lg",
								function() {
         ...
         */
						...
						Plugin.prototype.keyPress = function() {
							...
							_lgUtils2.default.on(window, "keydown.lg", function(
								e
							) {
								if (
									_this.s.escKey === true &&
						...
						Plugin.prototype.setTranslate = function(
							el,
							xValue,
							yValue
						) {
						...
						Plugin.prototype.touchMove = function(
							...
							if (Math.abs(distance) > 15) {
								...
								this.setTranslate(
									...
									-this.___slide[this.index].clientWidth +
										distance,
									0
								);
								this.setTranslate(
						...
						Plugin.prototype.destroy = function(d) {
							...
							setTimeout(function() {
								try {
									if (_this.outer) {
										_this.outer.parentNode.removeChild(
											_this.outer
										);
									}
