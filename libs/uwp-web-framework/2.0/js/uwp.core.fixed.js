...
(function (root, document) {
	...
	var UWP = {
		...
		init: function init(params) {
			...
			UWP.getNavigation();
			UWP.createStyles();
			UWP.navigate(root.location.hash.split(UWP.config.hashBang)[1], false);
		...
		getNavigation: function getNavigation(target) {
			...
			function parseNavElement(el) {
				...
				navLink.href = "".concat(UWP.config.hashBang, navTarget);
			...
			var UWP_request = root.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
			UWP_request.overrideMimeType("text/html;charset=utf-8");
			UWP_request.open("GET", URL, true);
			UWP_request.setRequestHeader("X-Requested-With", "XMLHttpRequest");
			UWP_request.withCredentials = false;
			var UWP_request_handler = function () {
				if (UWP_request.status === 404 || UWP_request.status === 0) {
					return;
				} else if (UWP_request.readyState === 4 && UWP_request.status === 200 && UWP_request.responseText) {
					var parsed = parseDomFromString(UWP_request.responseText);
			...
			UWP_request.addEventListener("readystatechange", UWP_request_handler, false);
			UWP_request.send(null);
		},
		...
		navigate: function navigate(target, addHistory) {
			...
			function displayError(title, linkText) {
				UWP.main.innerHTML = "\n\t<div class=\"uwp-error\">\n\t<p>".concat(title, "</p>\n\t<p><a href=\"", UWP.config.hashBang, UWP.config.home, "\" class=\"uwp-error-link\">" + linkText + "</a></p>\n\t</div>\n\t");
			...
			var UWP_navigate = root.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
			UWP_navigate.overrideMimeType("text/html;charset=utf-8");
			UWP_navigate.open("GET", URL, true);
			UWP_navigate.setRequestHeader("X-Requested-With", "XMLHttpRequest");
			UWP_navigate.withCredentials = false;
			var UWP_navigate_handler = function () {
				if (UWP_navigate.status === 404 || UWP_navigate.status === 0) {
					console.log("Error XMLHttpRequest-ing file", UWP_navigate.status);
					console.error(UWP.config.errorTitle);
					displayError(UWP.config.errorTitle, UWP.config.errorLinkText);
				} else if (UWP_navigate.readyState === 4 && UWP_navigate.status === 200 && UWP_navigate.responseText) {
					var parsed = parseDomFromString(UWP_navigate.responseText);
			...
			UWP_navigate.addEventListener("readystatechange", UWP_navigate_handler, false);
			UWP_navigate.send(null);
		}
	};
	root.UWP = UWP;
