		...
		if (h) {
			return d.prototype = {
				...
				initTimer: function (a) {
					var b = this;
					return b.timing ? (b.stopTimer(), e = b.isFinished() ? setInterval(function () {
								b.stopTimer();
								a.next();
				...
				for (var g = 0; g < i.length; g++)
					if (!i[g].id) {
						...
						if (e && /^(\d{1,2}:)?\d{1,3}$/.test(e)) {
							if (-1 !== e.indexOf(":")) {
								e = e.split(":");
								e = 1e3 * (60 * parseInt(e[0], 10) + parseInt(e[1], 10));
							} else {
								e = 1e3 * parseInt(e, 10);
								if (0 === e) {
									e = !1;
						...
						f.slideList.push(new d({
								id: i[g].id,
								number: g,
								hasInnerNavigation: null !== i[g].querySelector(".next"),
								timing: e,
								innerLength: i[g].querySelectorAll(".next").length,
								innerComplete: 0
							}));
					}
				return f;
			...
			f._getTransform = function () {
				var b = Math.max(h.clientWidth / a.innerWidth, h.clientHeight / a.innerHeight);
				return "scale(" + 1 / b + ")";
			},
			f._applyTransform = function (a) {
				return ["WebkitTransform", "MozTransform", "msTransform", "OTransform", "transform"].forEach(function (b) {
					h.style[b] = a;
				}),
				!0;
			},
			...
			f.enterSlideMode = function (a) {
				var b = f.getCurrentSlideNumber();
				return h.classList.remove(h_cls),
			...
			f.enterListMode = function (a) {
				var b;
				return h.classList.remove("full"),
			...
			f.scrollToSlide = function (c) {
				...
				if (-1 === c)
					return e;
				if (!f.slideList[c])
					throw new Error("There is no slide with number " + c);
				return d = b.getElementById(f.slideList[c].id),
				a.scrollTo(0, d.offsetTop),
				e = !0,
				e;
			},
			f.isListMode = function () {
				return k ? !/^full.*/.test(g.search.substr(1)) : h.classList.contains(h_cls);
			},
			f.isSlideMode = function () {
				return k ? /^full.*/.test(g.search.substr(1)) : h.classList.contains("full");
			},
			f.updateProgress = function (a) {
				...
				if (!f._isNumber(a))
					throw new Error("Gimme slide number as Number, baby!");
				return j.style.width = (100 / (f.slideList.length - 1) * f._normalizeSlideNumber(a)).toFixed(2) + "%",
				!0;
			},
			f.updateActiveAndVisitedSlides = function (a) {
				...
				if (a = f._normalizeSlideNumber(a), !f._isNumber(a))
					throw new Error("Gimme slide number as Number, baby!");
				for (c = 0; e > c; ++c)
			...
			f.showPresenterNotes = function (c) {
				if (f.clearPresenterNotes(), a.console) {
					...
					e = f.slideList[c + 1] ? f.slideList[c + 1].id : null,
					g = b.getElementById(d).querySelector("footer");
					if (g && g.innerHTML && console.info(g.innerHTML.replace(/\n\s+/g, "\n")), e) {
						var h = b.getElementById(e).querySelector("h2");
						if (h) {
							h = h.innerHTML.replace(/^\s+|<[^>]+>/g, "");
							console.info("NEXT: " + h);
						}
					}
				}
			...
			f.getSlideHash = function (a) {
				if (!f._isNumber(a))
					throw new Error("Gimme slide number as Number, baby!");
				return a = f._normalizeSlideNumber(a),
				"#" + f.slideList[a].id;
			},
			f.wheel = function (a) {
				var d,
				...
				if (!g) {
					if (!f.isListMode()) {
						...
						d = a.deltaY === c ? a.wheelDeltaY < 0 : a.deltaY > 0;
						if (d) {
							f._turnNextSlide();
						} else {
			...
			b.addEventListener("keydown", function (a) {
				...
				switch (a.which) {
				case 80:
					if (f.isListMode() && a.altKey && a.metaKey) {
						a.preventDefault();
						b = d.number;
						f.go(b);
						f.enterSlideMode();
				...
				case 32:
					a.preventDefault();
					f[a.shiftKey ? "_turnPreviousSlide" : "_turnNextSlide"]();
				}
			}, !1),
			f.init(),
			...
			b.addEventListener("touchstart", function (b) {
				...
				if (g && (f.isSlideMode() && !f._checkInteractiveElement(b))) {
					e = b.touches[0].pageX;
					if (e > a.innerWidth / 2) {
						f._turnNextSlide();
					} else {
